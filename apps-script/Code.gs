...
function publicBindings_() { const result = {}; const rows = bindingRows_(); rows.forEach((row, index) => { const id = String(row['ID Siswa'] || '').trim(); if (!id || bindingIndex_(rows, id) !== index) return; result[id] = { studentId: id, studentName: String(row['Nama Siswa'] || '').trim(), status: String(row['Status'] || '').trim().toUpperCase(), boundAt: iso_(row['Tanggal Bind']), resetAt: iso_(row['Tanggal Reset']), updatedAt: iso_(row['Updated At']) }; }); return result; }
function tokenInUse_(token, exceptId) { const wanted = bindingId_(exceptId); return bindingRows_().some(row => bindingId_(row['ID Siswa']) !== wanted && String(row['Status'] || '').trim().toUpperCase() === 'TERDAFTAR' && String(row['Device Token'] || '').trim() === String(token || '').trim()); }
function writeRow_(sheet, rowNumber, values) { sheet.getRange(rowNumber, 1, 1, values.length).setValues([values]); }
function bindingCheck_(payload) { const binding = binding_(payload.studentId); if (tokenInUse_(payload.deviceToken, payload.studentId)) return json_({ status: 'DEVICE_DIPAKAI', message: 'Perangkat ini sudah terdaftar untuk akun siswa lain.' }, null, 409); if (!binding) return json_({ status: 'BELUM_TERDAFTAR' }); if (String(binding.Status || '').trim().toUpperCase() === 'DI-RESET') return json_({ status: 'DI-RESET' }); if (!payload.deviceToken) return json_({ status: 'DEVICE_TIDAK_DIkenal', message: 'Perangkat tidak dikenali.' }, null, 409); if (String(binding['Device Token'] || '').trim() !== String(payload.deviceToken).trim()) return json_({ status: 'DEVICE_LAIN', message: 'Akun ini sudah terdaftar pada perangkat lain.' }, null, 409); return json_({ status: 'TERDAFTAR' }); }
function bindingBind_(payload) { if (!payload.studentId || !payload.deviceToken) return json_({ message: 'studentId dan deviceToken wajib diisi.' }, null, 400); if (tokenInUse_(payload.deviceToken, payload.studentId)) return json_({ message: 'Perangkat ini sudah terdaftar untuk akun siswa lain.' }, null, 409); const sheet = sheet_(SHEETS.bindings); const rows = bindingRows_(); const index = bindingIndex_(rows, payload.studentId); const current = index >= 0 ? rows[index] : null; if (current && String(current.Status || '').trim().toUpperCase() === 'TERDAFTAR' && String(current['Device Token'] || '').trim() !== String(payload.deviceToken).trim()) return json_({ message: 'Akun ini sudah terdaftar pada perangkat lain.' }, null, 409); const now = new Date(); const row = [payload.studentId, payload.studentName || current?.['Nama Siswa'] || '', payload.deviceToken, 'TERDAFTAR', current?.['Tanggal Bind'] || now, '', now]; if (index >= 0) writeRow_(sheet, index + 2, row); else sheet.appendRow(row); return json_({ status: 'TERDAFTAR' }); }
