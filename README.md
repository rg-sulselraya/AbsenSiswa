...
Spreadsheet yang diberikan telah dicek read-only. Sheet `Database Siswa` memiliki header `User Serial`, `Nama Siswa`, `No Ortu`, `Nama Sekolah`, `Grade`, dan `Kelas`; kolom `Nama Cabang` juga dapat digunakan untuk pemetaan cabang. Mapping backend: `User Serial → id`, `Nama Siswa → name`, `No Ortu → parentPhone`, `Grade → grade`, `Kelas → className`, `Nama Cabang → branch`. Nama cabang pada data siswa dicocokkan ke `Database Cabang` untuk memperoleh `branchId`; alternatifnya tetap dapat memakai sheet `Siswa Cabang`. Jangan mengubah data siswa existing. Jika diperlukan, gunakan sheet tambahan `Presensi` dengan kolom `Tanggal, ID Siswa, Nama Siswa, Kelas, ID Cabang, Cabang, Jam Datang, Jam Pulang, Status`, `Database Cabang` dengan kolom `ID Cabang, Nama Cabang, Status`, dan sheet `StatusWA` dengan kolom `Tanggal, ID Siswa, Status WA, Waktu Diproses, Waktu Terkirim, Jenis WA`. `Jenis WA` membedakan pesan `arrival` (jam datang) dan `departure` (jam pulang); baris lama tanpa jenis dianggap sebagai pesan jam datang.
...
- Student Mentor dapat membuat dan mencetak QR Cabang dinamis dari data `Database Cabang`; QR berisi ID cabang yang dipakai siswa saat presensi.
